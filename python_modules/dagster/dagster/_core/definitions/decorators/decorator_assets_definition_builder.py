...
def validate_can_coexist(asset_in: AssetIn, asset_dep: AssetDep) -> None:
    ...
    if (
        ...
        raise DagsterInvalidDefinitionError(
            f"Asset key '{asset_dep.asset_key.to_user_string()}' is used as both an input and a dependency. If an asset key is used as an input and also set as a dependency, the input should only define the relationship between the asset key and the input name, or optionally set the input_manager_key. Any other properties should either not be set, or should be set on the dependency."
        )
...
def validate_named_ins_subset_of_deps(
    ...
    if asset_in_keys - asset_dep_keys:
        ...
        raise DagsterInvalidDefinitionError(
            f"Invalid asset dependencies: `{invalid_asset_in_keys}` specified as asset inputs, but"
            " are not specified in `internal_asset_deps`. Asset inputs must be associated with an"
